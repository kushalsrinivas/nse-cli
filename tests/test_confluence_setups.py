...
class TestEngineJournaling(unittest.TestCase):
    ...
    def test_explicit_record_writes_hypothetical(self):
        ...
        self.assertTrue(all(r.timeframe == "5m" for r in rows))
...
class TestLotsAndTraded(unittest.TestCase):
    ...
    def test_migrate_adds_columns_to_old_db(self):
        import sqlite3

        from journal import confluence_db as cjmod
        old_path = Path(self.tmp_dir.name) / "old.db"
        old_schema = cjmod._SCHEMA.replace("    lots INTEGER NOT NULL DEFAULT 1,\n", "")
        old_schema = old_schema.replace("    timeframe TEXT NOT NULL DEFAULT '5m',\n", "")
        ...
        rec = cj.get(1)
        self.assertEqual(rec.setup_id, "B")
        # ALTER TABLE backfills existing rows with the column defaults.
        self.assertEqual((rec.lots, rec.timeframe), (1, "5m"))
