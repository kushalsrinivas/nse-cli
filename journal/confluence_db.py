...
class ConfluenceJournal:
    ...
    def _migrate(self) -> None:
        """Add post-v1 columns to databases created before qty/timeframe."""
        existing = {r["name"] for r in self.conn.execute("PRAGMA table_info(confluence_trade_journal)")}
        for col, decl in _MIGRATIONS:
            if col not in existing:
                self.conn.execute(f"ALTER TABLE confluence_trade_journal ADD COLUMN {col} {decl}")
        self.conn.commit()
